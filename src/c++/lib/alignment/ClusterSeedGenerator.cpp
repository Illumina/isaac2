                ...
                if (flowcell::BarcodeMetadata::UNMAPPED_REFERENCE_INDEX != referenceIndex)
                {
                    BOOST_FOREACH(const SeedMetadata &seedMetadata, BaseT::seedMetadataOrderedByFirstCycle_)
                    {
                        ...
                        if (!clustersToDiscard.at(clusterId).isReadComplete(readIndex))
                        {
                            ...
                            forwardSeed = Seed<KmerT>(KmerT(0), SeedId(currentTile->getIndex(), barcodeIndex, clusterId, seedMetadata.getIndex(), 0));
...
//                                    }
            ...
            ISAAC_THREAD_CERR << "Generating seeds done for " << *currentTile << " generated " <<
                (totalSeeds - nSeeds) << " normal and " << nSeeds << " n-seeds" << std::endl;
        }
    }
}

template class ClusterSeedGenerator<oligo::ShortKmerType>;
template class ClusterSeedGenerator<oligo::BasicKmerType<28> >;
template class ClusterSeedGenerator<oligo::BasicKmerType<30> >;
template class ClusterSeedGenerator<oligo::KmerType>;
template class ClusterSeedGenerator<oligo::BasicKmerType<34> >;
template class ClusterSeedGenerator<oligo::BasicKmerType<36> >;

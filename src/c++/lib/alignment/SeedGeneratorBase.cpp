...
{
    nextTileSeedBegins_.reserve(sortedReferenceMetadataList.size());
...
{
    // keep the minimum of forward and reverse kmer. This is what we have in sorted reference.
    ISAAC_THREAD_CERR << "Computing reverse complements for " << seeds.size() << " seeds" << std::endl;
    BOOST_FOREACH(Seed<KmerT> &seed, seeds)
    {
        if (!seed.isNSeed())
        {
            const KmerT rev = oligo::reverseComplement(seed.kmer());
            if (rev < seed.kmer())
            {
                seed.setReverse();
                seed.kmer() = rev;
    ...
    ISAAC_THREAD_CERR << "Computing reverse complements for " << seeds.size() << " seeds done" << std::endl;
    ...
    BOOST_FOREACH(const std::vector<std::vector<unsigned> > &reference, referenceTileReadFragmentCounts_)
    {
        ...
        BOOST_FOREACH(const flowcell::ReadMetadata &readMetadata, flowcellLayout_.getReadMetadataList())
        {
            ...
            nextTileSeedBegins_.at(referenceIndex) +=
                reference.at(tileIndex).at(readIndex) * seedCounts_.at(readIndex);
        }
        ...
        BOOST_FOREACH(const flowcell::TileMetadata &tile, tiles)
        {
            BOOST_FOREACH(const flowcell::ReadMetadata &read, flowcellLayout_.getReadMetadataList())
            {
                referenceTilesBeginIterator +=
                    reference.at(tile.getIndex()).at(read.getIndex()) * seedCounts_.at(read.getIndex());
            }
        }
    }
...
template class SeedGeneratorBase<oligo::BasicKmerType<28> >;
template class SeedGeneratorBase<oligo::BasicKmerType<30> >;
template class SeedGeneratorBase<oligo::KmerType>;
template class SeedGeneratorBase<oligo::BasicKmerType<34> >;
template class SeedGeneratorBase<oligo::BasicKmerType<36> >;

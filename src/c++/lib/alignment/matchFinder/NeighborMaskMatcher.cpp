    ...
    std::istream &reference)
{
    ...
    if (reference)
    {
        while(endSeeds != nextSeed)
        {
            ...
            {
                ISAAC_ASSERT_MSG(threadRepeatList.size() <= repeatThreshold_, "Collected more repeats than allowed: " << threadRepeatList.size() << " max:" << repeatThreshold_);
...
template class NeighborMaskMatcher<oligo::BasicKmerType<28> >;
template class NeighborMaskMatcher<oligo::BasicKmerType<30> >;
template class NeighborMaskMatcher<oligo::KmerType>;
template class NeighborMaskMatcher<oligo::BasicKmerType<34> >;
template class NeighborMaskMatcher<oligo::BasicKmerType<36> >;

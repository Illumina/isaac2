 ...
 ** Corresponding probability of a base being incorrectly sequenced.
 **/
std::vector<double> getLogErrorLookup()
{
...
std::vector<double> getLogMismatchLookup()
{
    ...
    {
        const double logMismatch = Quality::getLogMismatchSlow(quality);
        lookup.push_back(logMismatch);
    }
    return lookup;
}

const std::vector<double> Quality::logErrorLookup = getLogErrorLookup();

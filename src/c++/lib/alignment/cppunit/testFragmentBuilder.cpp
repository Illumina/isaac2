...
{
    cluster0.init(readMetadataList, bcl0.begin(), tile0, clusterId0, isaac::alignment::ClusterXy(0,0), true, 0, 0);
    cluster2.init(readMetadataList, bcl2.begin(), tile2, clusterId2, isaac::alignment::ClusterXy(0,0), true, 0, 0);
    cluster3.init(readMetadataList, bcl3.begin(), tile0, clusterId0, isaac::alignment::ClusterXy(0,0), true, 0, 0);
    cluster4l.init(readMetadataList, bcl4l.begin(), tile0, clusterId0, isaac::alignment::ClusterXy(0,0), true, 0, 0);
    cluster4t.init(readMetadataList, bcl4t.begin(), tile0, clusterId0, isaac::alignment::ClusterXy(0,0), true, 0, 0);
    cluster4lt.init(readMetadataList, bcl4lt.begin(), tile0, clusterId0, isaac::alignment::ClusterXy(0,0), true, 0, 0);
...
void TestFragmentBuilder::testEmptyMatchList()
{
    ...
    FragmentBuilder fragmentBuilder(flowcells, 123, seedMetadataList.size()/2, 8, 2, false, false, false, alignmentCfg);
    ...
    fragmentBuilder.build(contigList, contigAnnotations, readMetadataList, seedMetadataList, testAdapters, isaac::alignment::TemplateLengthStatistics(),
...
void TestFragmentBuilder::auxSingleSeed(const unsigned s0, const unsigned s1)
{
    ...
    isaac::alignment::AlignmentCfg alignmentCfg(ELAND_MATCH_SCORE, ELAND_MISMATCH_SCORE, ELAND_GAP_OPEN_SCORE, ELAND_GAP_EXTEND_SCORE, ELAND_MIN_GAP_EXTEND_SCORE, 20000);
    FragmentBuilder fragmentBuilder(flowcells, 456, seedMetadataList.size()/2, 8, 2, false, false, true, alignmentCfg);
    // build the fragments
    fragmentBuilder.build(contigList, contigAnnotations, readMetadataList, seedMetadataList, testAdapters, isaac::alignment::TemplateLengthStatistics(), matchList.begin(), matchList.end(), cluster0, true);
...
void TestFragmentBuilder::testMultiSeed()
{
    ...
    using isaac::alignment::Cigar;
    isaac::alignment::AlignmentCfg alignmentCfg(ELAND_MATCH_SCORE, ELAND_MISMATCH_SCORE, ELAND_GAP_OPEN_SCORE, ELAND_GAP_EXTEND_SCORE, ELAND_MIN_GAP_EXTEND_SCORE, 20000);
    FragmentBuilder fragmentBuilder(flowcells, 123, seedMetadataList.size()/2, 8, 2, false, false, true, alignmentCfg);
    // build the fragments
    fragmentBuilder.build(contigList, contigAnnotations, readMetadataList, seedMetadataList, testAdapters, isaac::alignment::TemplateLengthStatistics(), matchList.begin(), matchList.end(), cluster0, true);
    // check buffer geometry
    CPPUNIT_ASSERT_EQUAL((size_t)2, fragmentBuilder.getFragments().size());
...
void TestFragmentBuilder::testRepeats()
{
    ...
    FragmentBuilder fragmentBuilder(flowcells, 123, seedMetadataList.size()/2, 8, 2, false, false, true, alignmentCfg);
    // build the fragments
    fragmentBuilder.build(contigList, contigAnnotations, readMetadataList, seedMetadataList, testAdapters, isaac::alignment::TemplateLengthStatistics(), matchList.begin(), matchList.end(), cluster2, true);
    // check buffer geometry
    CPPUNIT_ASSERT_EQUAL((size_t)2, fragmentBuilder.getFragments().size());
...
void TestFragmentBuilder::testMismatches()
{
    ...
    FragmentBuilder fragmentBuilder(flowcells, 123, seedMetadataList.size()/2, 8, 2, false, false, true, alignmentCfg);
    // build the fragments
    fragmentBuilder.build(contigList, contigAnnotations, readMetadataList, seedMetadataList, testAdapters, isaac::alignment::TemplateLengthStatistics(), matchList.begin(), matchList.end(), cluster3, true);
    // check buffer geometry
    CPPUNIT_ASSERT_EQUAL((size_t)2, fragmentBuilder.getFragments().size());
    CPPUNIT_ASSERT_EQUAL((size_t)2, fragmentBuilder.getCigarBuffer().size());
    // Fragment for the first read (forward)
...
void TestFragmentBuilder::testLeadingSoftClips()
{
    ...
    FragmentBuilder fragmentBuilder(flowcells, 123, seedMetadataList.size()/2, 8, 2, false, false, true, alignmentCfg);
    // build the fragments
    fragmentBuilder.build(contigList, contigAnnotations, readMetadataList, seedMetadataList, testAdapters, isaac::alignment::TemplateLengthStatistics(), matchList.begin(), matchList.end(), cluster4l, true);
    // Fragment for the first read (forward)
...
void TestFragmentBuilder::testTrailingSoftClips()
{
    ...
    FragmentBuilder fragmentBuilder(flowcells, 123, seedMetadataList.size()/2, 8, 2, false, false, true, alignmentCfg);
    // build the fragments
    fragmentBuilder.build(contigList, contigAnnotations, readMetadataList, seedMetadataList, testAdapters, isaac::alignment::TemplateLengthStatistics(), matchList.begin(), matchList.end(), cluster4t, true);
    // Fragment for the first read (forward)
...
void TestFragmentBuilder::testLeadingAndTrailingSoftClips()
{
    ...
    FragmentBuilder fragmentBuilder(flowcells, 123, seedMetadataList.size()/2, 8, 2, false, false, true, alignmentCfg);
    // build the fragments
    fragmentBuilder.build(contigList, contigAnnotations, readMetadataList, seedMetadataList, testAdapters, isaac::alignment::TemplateLengthStatistics(), matchList.begin(), matchList.end(), cluster4lt, true);
    // Fragment for the first read (forward)
    CPPUNIT_ASSERT_EQUAL((long)0, fragmentBuilder.getFragments()[0][0].position);

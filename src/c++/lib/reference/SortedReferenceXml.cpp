...
void serialize(xml::XmlReader &reader, SortedReferenceMetadata::AnnotationFile &af, const unsigned int version)
{
    ...
    if ("16bpb" != annotationFormat)
    {
        BOOST_THROW_EXCEPTION(xml::XmlReaderException(std::string("Only 16bpb annotations are supported. got: ") + annotationFormat));
    }
...
{
    while (reader.nextElementBelowLevel(2) && reader("Annotation"))
    {
        if (reader["Type"] != "k-uniqueness")
        {
            BOOST_THROW_EXCEPTION(xml::XmlReaderException(std::string("Only k-uniqueness annotations are supported")));
        }
        // TODO: make sure the rest of the toolkit can handle multiple annotations (score filtering, pack/unpack, reorder reference)
        ISAAC_ASSERT_MSG(annotationFiles.empty(), "Multiple annotations are not supported");
        ...
        if (!sortedReferenceMetadata.annotationFiles_.empty())
        {
            ISAAC_XML_WRITER_ELEMENT_BLOCK(writer, "Annotations")
            {
                              ...
                              sortedReferenceMetadata.annotationFiles_)
                {
                    ...
                    {
                        writer.writeAttribute("Type", "k-uniqueness");
                        writer.writeAttribute("K", annotation.k_);
                        writer.writeElement("Format", "16bpb");

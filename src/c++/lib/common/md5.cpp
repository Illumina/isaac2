...
ncluded to satisfy legal requirements.
...
void MD5Digest(MD5Context *md5, const void *input, unsigned int inputLen)
{
  ...
  if (inputLen >= partLen)
  {
    ...
    {
      // MD5Transform(md5->state, (const unsigned char *) input + i);
      MD5Transform(md5->state, (const unsigned char*) input + i);
    }
    index = 0;
  }
...
void MD5Close(MD5Context *md5, unsigned char digest[16])
{
  ...
  /* Zeroize sensitive information. */
  memset(md5, 0, sizeof(MD5Context));
}
//#include <fstream>
//#include <sstream>
//#include <iostream>
...
//            MD5Digest(&md5, szBuffer, is.gcount());
//        }
...
//        {
//            const int val = static_cast<int>(digest[i]);
//            sss << ((val < 16)?("0"):("")) << std::hex << val;
//        }
...
//        {
//            is.read(szBuffer, sizeof(szBuffer));
//            MD5_Update(&md5, szBuffer, is.gcount());
//        }
//        unsigned char digest[16];
